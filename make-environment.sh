...
then
    ...
    gcloud beta container --project ${PROJECT_ID} clusters create ${CLUSTER_NAME} --zone "us-central1-c" --no-enable-basic-auth --cluster-version "1.16.13-gke.401" --machine-type "e2-small" --image-type "COS" --disk-type "pd-standard" --disk-size "10" --metadata disable-legacy-endpoints=true --scopes "https://www.googleapis.com/auth/cloud-platform" --num-nodes ${NUM_NODES} --enable-stackdriver-kubernetes --enable-ip-alias --network "projects/${PROJECT_ID}/global/networks/default" --subnetwork "projects/${PROJECT_ID}/regions/us-central1/subnetworks/default" --default-max-pods-per-node "110" --no-enable-master-authorized-networks --addons HorizontalPodAutoscaling,HttpLoadBalancing --enable-autoupgrade --enable-autorepair --max-surge-upgrade 1 --max-unavailable-upgrade 0
    gcloud container clusters get-credentials ${CLUSTER_NAME} -z us-central1-c
fi
...
sed -e "s/\${tag_id}/${TAG_ID}/" -e "s/\${project_id}/${PROJECT_ID}/" -e "s/\${topic_name}/${TOPIC_NAME}/" -e "s/\${tw_key}/${secrets[0]}/" -e "s/\${tw_secret}/${secrets[1]}/" -e "s/\${tw_token}/${secrets[2]}/" -e "s/\${tw_token_sec}/${secrets[3]}/" -e "s/\${tw_keywords}/${TW_KEYWORDS}/"  pubsub/twitter-stream-template.yaml > twitter-stream.yaml